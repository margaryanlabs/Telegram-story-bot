...
export async function listBusinessConnectionCandidates(limit = 50) {
  return (await callStore('list_business_connection_candidates', {
...
export async function getBusinessConnectionState(userId) {
  return callStore('get_business_connection_state', {
...
export async function upsertBusinessConnectionState(row) {
  return callStore('upsert_business_connection_state', {
    row,
  }, {
    delays: [0, 180],
    timeoutMs: 4000,
...
export async function listDeletedFeed(userId, limit = 120) {
  return (await callStore('list_deleted_feed', {
    userId: String(userId),
    limit: Math.max(1, Math.min(160, Number(limit || 120))),
  }, {
    delays: [0],
    timeoutMs: 4500,
  })) || { items: [] };
