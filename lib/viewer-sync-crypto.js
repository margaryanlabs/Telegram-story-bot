...
import { openViewerPrivateJson, sealViewerPrivateJson } from './viewer-sync-store.js';
...
export async function sealJson(value, context) {
  return sealViewerPrivateJson(value, context);
}
...
export async function openJsonWithMeta(value, context) {
  ...
  if (version === VERSION_V3) {
    return {
      value: await openViewerPrivateJson(value, context),
      version: VERSION_V3,
      keyId: parts[1] || null,
      legacy: false,
    };
...
export function isLegacyCiphertext(value) {
  const version = String(value || '').split('.')[0];
  return version === VERSION_V1 || version === VERSION_V2;
}
